...
const buildCiViewConfigRows = (): ViewConfigRow[] => {
   ...
   *
   * @param {number} viewId - Stable identity for this seed row (CI has no DB identity column).
  ...
  const buildRow = (
    ...
    viewType,
    primaryDataset,
    secondaryDataset: deriveSecondaryDataset(viewType, config),
    viewConfig: config,
  });

  return [
    buildRow(1, "seed_survey_data", "gallery", surveyConfig),
    buildRow(2, "bcmform_responses", "map", bcmformConfig),
    buildRow(3, "bcmform_responses", "gallery", bcmformConfig),
    buildRow(4, "fake_alerts", "alerts", fakeAlertsConfig),
    buildRow(5, "gfw_alerts_viirs", "alerts", gfwAlertsConfig),
  ];
};
