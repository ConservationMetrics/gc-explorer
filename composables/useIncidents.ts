...
export const useIncidents = (
  ...
  /**
   * Returns current alerts dataset name.
   *
   * @returns Alerts dataset table name or undefined.
   */
...
const SOURCE_ID_KEYS = ['alertID', '_id', 'source_id', 'sourceId'] as const;
   ...
   * - For alert layers (containing "most-recent-alerts" or "previous-alerts"): Uses the configured primary dataset; feature_type "alert".
   * - For Mapeo layers (layerId.startsWith("mapeo-data")): Uses the configured secondary dataset; feature_type "mapeo".
   *
  ...
  const handleMultiSelectFeature = (feature: Feature, layerId: string) => {
    ...
    if (
      ...
      sourceTable = currentAlertsTable || "";
      featureType = "alert";
